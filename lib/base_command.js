...
const USER_HOME = require('./adapter/user_home');

const configDir = path.join(process.env[USER_HOME], '.projj');
...
const defaults = {
  base: `${process.env[USER_HOME]}/projj`,
  hooks: {},
  alias: {
...
function resolveConfig(config, defaults) {
  ...
  config.base = config.base.map(baseDir => {
    switch (baseDir[0]) {
      ...
      case '~':
        return baseDir.replace('~', process.env[USER_HOME]);
      case '/':
        return baseDir;
      default:
